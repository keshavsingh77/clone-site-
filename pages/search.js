...
import { geminiService } from './api/search';
...
const SearchPage = () => {
  ...
  const performSearch = async () => {
    setLoading(true);
    const searchResults = await geminiService.searchPosts(query);
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {results.map(({ post, relevanceReason }) => (
            <div key={post.id} className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm hover:shadow-md transition-shadow">
              ...
              <a href={`#/post/${post.id}`} className="text-sm font-bold text-indigo-600 hover:underline">Read Post →</a>
            </div>
          ))}
        </div>
      ) : (
